...
peak=GenomicRanges::GenomicRangesList(FB=readPeakFile(files[[13]]),
                                      FA=readPeakFile(files[[7]]),
                                      MB=readPeakFile(files[[28]]),
                                      MA=readPeakFile(files[[21]]),
                                      SB=readPeakFile(files[[43]]),
                                      SA=readPeakFile(files[[36]]),
                                      VB=readPeakFile(files[[59]]),
                                      VA=readPeakFile(files[[51]]))

p <- covplot(peak, weightCol=5, title = "H3K4me3 Peaks over Chromosomes", chrs=c("DDB0232428", "DDB0232429", "DDB0232430", "DDB0232431", "DDB0232432", "DDB0232433" ))
