...
peak=GenomicRanges::GenomicRangesList(FB=readPeakFile(files[[11]]),
                                      FA=readPeakFile(files[[5]]),
                                      MB=readPeakFile(files[[26]]),
                                      MA=readPeakFile(files[[19]]),
                                      SB=readPeakFile(files[[41]]),
                                      SA=readPeakFile(files[[34]]),
                                      VB=readPeakFile(files[[57]]),
                                      VA=readPeakFile(files[[49]]))
p <- covplot(peak, weightCol=5, title = "H3K36me3 Peaks over Chromosomes", chrs=c("DDB0232428", "DDB0232429", "DDB0232430", "DDB0232431", "DDB0232432", "DDB0232433" ))
