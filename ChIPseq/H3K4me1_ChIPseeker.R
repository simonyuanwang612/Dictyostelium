...
peak=GenomicRanges::GenomicRangesList(FB=readPeakFile(files[[12]]),
                                      FA=readPeakFile(files[[6]]),
                                      MB=readPeakFile(files[[27]]),
                                      MA=readPeakFile(files[[20]]),
                                      SB=readPeakFile(files[[42]]),
                                      SA=readPeakFile(files[[35]]),
                                      VB=readPeakFile(files[[58]]),
                                      VA=readPeakFile(files[[50]]))

p <- covplot(peak, weightCol=5, title = "H3K4me1 Peaks over Chromosomes", chrs=c("DDB0232428", "DDB0232429", "DDB0232430", "DDB0232431", "DDB0232432", "DDB0232433" ))
